...
def format_well_info_table(df):
    ...
    html_table = df.to_html(
        escape=False, index=False, header=False, classes="styled-table"
    )
    return html_table
...
def update_year_options(event):
    ...
    year_range_slider.param.set_param(value=(years[-1], years[0]))
...
def update_view(station, year_range):
    ...
    plot_precipitation = gw.get_precipitation_chart(
        df_prec, settings
    )  # gw.get_precipitation_chart(year_range)
    # Table
    table = pn.widgets.Tabulator(
        df[["stationnr", "date", "value"]].rename(
            columns={
                "stationnr": "Station",
                "date": "Datum",
                "value": "Wasserstand [m.ü.M.]",
            }
        ),
        sizing_mode="stretch_both",
        name="Table",
        show_index=False, # not working, why
    )
    ...
    info_panel = pn.pane.HTML(
        html, sizing_mode="stretch_width", stylesheets=[stylesheet]
    )
    styles = {
    ...
    return pn.Tabs(
        ("📈 Plot", pn.Column(pn.pane.Vega(plot), pn.pane.Vega(plot_precipitation))),
        ("📊 Tabelle", table),
        ("🗺️ Karte", map_panel),
...
page = pn.template.MaterialTemplate(
    title="⛲ Grundwassermonitoring Basel-Stadt", sidebar=sidebar, main=[tabs]
)
page.servable()
